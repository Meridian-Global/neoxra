...
const DEFAULT_THREAD: ThreadsThread = {
  format: 'thread',
  ...
  posts: [
    {
      post_number: 1,
      purpose: 'hook',
      content: '多數小團隊不是缺內容點子，而是缺一個不會拖慢大家的內容節奏。',
    },
    {
      post_number: 2,
      purpose: 'argument',
      content: '如果每篇內容都要重新討論角度、格式、語氣和審稿標準，AI 只會讓混亂變快。',
    },
    {
      post_number: 3,
      purpose: 'evidence',
      content: '真正有效的流程通常很簡單：一個主題，一個明確受眾，三個平台版本，一次人工審稿。',
    },
    {
      post_number: 4,
      purpose: 'cta',
      content: '你現在的內容流程，最卡的是想法、產出，還是審稿？',
    },
  ],
}
...
function friendlyError(error: unknown) {
  if (error instanceof APIError) {
    if (error.status === 503) return 'Threads 生成服務目前尚未開啟，請稍後再試。'
    if (error.status === 422) return '請輸入更明確的主題。'
  }
  if (error instanceof Error && error.message.includes('timed out')) {
    return '生成時間過長，請重新嘗試。'
  }
  return '目前無法產生 Threads 內容，請稍後再試。'
}
...
export default function ThreadsPage() {
  ...
  const [thread, setThread] = useState<ThreadsThread>(DEFAULT_THREAD)
    ...
    try {
      for await (const chunk of streamSSE(
        ...
        if (chunk.event === 'error') {
          throw new Error(typeof chunk.data?.message === 'string' ? chunk.data.message : 'Threads 生成失敗。')
        }
      }
      if (!abort.signal.aborted) {
        throw new Error('Threads 生成尚未完成，串流已提早結束。')
      }
    } catch (err) {
      if (!abort.signal.aborted) {
        setStatus('error')
          ...
          <aside className="space-y-6 rounded-[20px] border border-[var(--border)] bg-[var(--bg-elevated)] p-6 shadow-[var(--shadow-sm)] lg:sticky lg:top-24 lg:self-start">
            <div>
              ...
              <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">
                輸入一個主題，產出單則貼文或一串可直接發布的 Threads。每則都會檢查 500 字限制。
              </p>
            </div>
