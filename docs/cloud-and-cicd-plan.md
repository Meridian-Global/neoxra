...
1. **Domain 首選**：`useorchestra.com`（品牌型、SaaS 慣例、可註冊率高）；
   備選 `tryorchestra.com` / `orchestra.so`；
   `orchestra.ai` 價錢通常 $2k~$10k，MVP 階段不建議搶。
2. **最佳架構組合**：
   - `orchestra` 前端 → **Vercel**
   - `orchestra` 後端 FastAPI → **GCP Cloud Run**（或 Railway，二擇一，見 B 段）
   - `orchestra-core` → **作為 Python package，不獨立部署**
   - `orchestra-linkedin` → **Chrome extension 只呼叫 orchestra API**，不自建後端
3. **未來 30 天重點**：
   - 第 1 週：買 domain、Vercel 部前端、Cloud Run 部後端（手動）
   - 第 2 週：把 orchestra-core 改成可 `pip install git+...` 的 package，orchestra 匯入它
   - 第 3 週：寫 GitHub Actions（三個 repo 各一份），main merge → auto deploy
   - 第 4 週：加 staging 環境、secrets 規劃、monitoring（Cloud Run + Sentry 免費方案）
4. **現在不要做**：自建 Kubernetes、GCP VPC / Cloud SQL、orchestra-core 獨立部署、前後端拆微服務、自己搞 Nginx。
...
3. **避開商標衝突**：`orchestra` 是常見字，單獨存在 SaaS 市場已被佔用（例：Orchestra 是一個 data ops 公司），因此原生 `orchestra.com` 幾乎不可能拿到。
...
| # | Domain | 優點 | 缺點 | 定位 |
|---|--------|------|------|------|
| 1 | `useorchestra.com` | SaaS 命名慣例（use-/try-/get-），可註冊率高，保留 orchestra 概念 | 前綴讓名字變長 | ✅ **長期品牌首選** |
| 2 | `tryorchestra.com` | 友好、實驗感、對早期使用者親切 | "try" 有暫時感，品牌化後可能要換 | ✅ 適合 MVP~中期 |
| 3 | `orchestra.so` | 短、現代 SaaS 感（Notion / Linear 都用 .so / 類似） | `.so` 辨識度仍遜於 `.com` | ✅ 長期可用 |
| 4 | `meridian.global` | 你公司名（Meridian Global），可當母品牌 | 不直接指向 orchestra 產品 | 🔸 當 corporate site，不當產品域名 |
| 5 | `playorchestra.com` | 活潑、有音樂 metaphor 延伸 | "play" 容易被誤以為遊戲 | 🔸 備選 |
...
| 8 | `postconductor.com` | conductor 呼應 orchestra metaphor | 略長 | 🔸 有創意，但較小眾 |
...
| # | Domain | 優點 | 缺點 | 定位 |
|---|--------|------|------|------|
| 10 | `orchestra-ai.com` | 直接連結 AI，你原本偏好 | 重複字（orchestra + ai），讀起來略拗口；.com 後綴抬價 | 🔸 可用但非最佳 |
| 11 | `getorchestra.com` | 與 useorchestra 類似，get- 前綴 SaaS 慣例 | 跟 useorchestra 重複度高，二選一 | 🔸 useorchestra 的備選 |
| 12 | `harmonize.ai` | 呼應 orchestra 概念、抽象、有想像空間 | 與許多既有品牌字重疊 | ✅ 有潛力，需查商標 |
...
```
                適合 MVP          適合長期品牌
品牌型        tryorchestra      useorchestra / orchestra.so
功能型        onebrief          （不推長期用功能型）
AI/SaaS 型    orchestra-ai      harmonize.ai
多平台擴充型   onetake           polyvoice
```

### 具體建議

- **現在就買：** `useorchestra.com`（主）+ `orchestra.so`（副）+ `tryorchestra.com`（防搶）。全部加起來一年約 $40~$60。
- **先不要碰：** `.ai` 網域通常每年 $70+，而且 `orchestra.ai` 多半要拍賣四位數以上。品牌未穩前不划算。
- **長期策略：** 產品站用 `useorchestra.com`，母公司保留 `meridian.global`，需要 API 子網域時再切 `api.useorchestra.com`、`app.useorchestra.com`。
...
| **5. orchestra-core 作 library** | $0 額外 | 低 | 低 | — | ✅ **必選** | ✅ 需要時再切 service | ✅ 對應 monorepo 哲學 |
| **6. orchestra-core 獨立 microservice** | +$10~$30 | 高 | 高 | 慢 | ❌ 太早 | ✅ | ❌ 早期不需要 |
...
#### 為什麼不推方案 6（orchestra-core 獨立部署）

現階段 orchestra-core 是 **能力函式庫**（skills、prompts、voice loader、LLM provider wrapper），不是會被多個 client 同時 HTTP 呼叫的服務。當你只有 1 個 client（orchestra backend）時，HTTP 包一層只會徒增延遲、錯誤來源、部署複雜度。**Library 模式的退出成本極低**——哪天真的需要獨立 service，把 skills 匯到 FastAPI endpoint 就是幾小時的工作。
...
### C2. orchestra-core（AI 能力層）

- **現階段：Library，不要獨立部署。**
- **做法：** 把 orchestra-core 發成可安裝的 Python package：
  - 方法 A（最快）：`orchestra` 的 `requirements.txt` 加一行
    ```
    orchestra-core @ git+https://github.com/<你的帳號>/orchestra-core.git@main
    ```
...
- **什麼時候改成 microservice？** 出現以下任一條件才考慮：
  1. orchestra-linkedin extension 要直接呼叫 AI（繞過 orchestra backend）
  2. 有第二個產品（非 orchestra）要用同一套 AI 能力
  3. AI 部分要獨立 scale（例如 GPU 或長任務佇列）
- **CI：** 只要跑 `pytest` + 打 tag 即可，不需要 deploy step。

### C3. orchestra-linkedin（LinkedIn 應用層）

- **不需要獨立後端伺服器。** Chrome extension 的特性是：UI 跑在瀏覽器，「後端」就是呼叫 `api.useorchestra.com/v1/...`。
- **部署其實只有兩件事：**
...
```
┌─────────────────────┐
│  useorchestra.com   │  ← Vercel（Next.js）
│  (marketing + app)  │
└──────────┬──────────┘
           │ fetch
           ▼
┌─────────────────────┐
│ api.useorchestra.com│  ← Cloud Run（FastAPI）
│  orchestra backend  │
│  │                  │
│  └─ orchestra-core  │  ← 同一個 Docker image 裡的 Python package
│     (as library)    │
└──────────┬──────────┘
           │ HTTPS
           ▼
...
Chrome extension (orchestra-linkedin)
  → 呼叫 api.useorchestra.com
```
...
- run: gcloud builds submit --tag gcr.io/$PROJECT/orchestra-backend
- run: gcloud run deploy orchestra-backend \
         --image gcr.io/$PROJECT/orchestra-backend \
         --region asia-east1 \
...
| `VERCEL_TOKEN` | GitHub Secrets（只有 orchestra 前端需要） | 前端部署 |
...
> 買 `useorchestra.com`；前端 Vercel，後端 Cloud Run，orchestra-core 當 library，orchestra-linkedin 只 build zip；main merge 就 auto deploy；staging/production 現在先不拆。

### E2. 未來 30 天執行順序

**Week 1：上線 demo（優先做「看得到」的事）**
- Day 1：註冊 `useorchestra.com`（Cloudflare Registrar 或 Namecheap，選 Cloudflare 有更好的 DNS UX）
- Day 2：orchestra 的 `frontend/` 綁 Vercel，PR preview 通過即 merge
- Day 3：寫 orchestra backend 的 `Dockerfile`；本地 `docker build` + `docker run` 確認能跑
- Day 4：建 GCP 專案 → 開 Cloud Run → 手動部一次，拿到 `api-xxx.run.app` URL
- Day 5：設 custom domain `api.useorchestra.com` 指到 Cloud Run
- Day 6~7：前端串 backend API，第一個 end-to-end demo（輸入 idea → 回傳三平台內容）

**Week 2：打底 orchestra-core 作 library**
- Day 8~9：在 orchestra-core 發 tag `v0.1.0`，測試 `pip install git+...` 從 orchestra 匯入
- Day 10~11：orchestra backend 改用 orchestra-core 的 skills / voice loader，刪掉重複程式碼
- Day 12~14：寫 orchestra-core 的 `ci.yml`，每次 push 跑 pytest

**Week 3：CI/CD 自動化**
- Day 15~17：寫 orchestra 的 `deploy.yml`，merge to main → 自動 build + push + deploy Cloud Run
- Day 18~19：前端 Vercel 的 auto deploy 驗證；設 preview URL 給自己當 staging
- Day 20~21：寫 orchestra-linkedin 的 build + zip action
...
2. **不要把 orchestra-core 拆成獨立服務**——現在只有一個 client，拆了只是徒增 bug 和延遲。
...
7. **不要做 orchestra-linkedin 的自動上架**——Chrome Web Store 審核時間決定節奏，自動化反而難 debug。
...
4. **orchestra-core 做好 pytest**：這是你之後重構、提取 skills 的根基。壞 test 比沒 test 更慘，現在還少就打好基礎。
...
5. **先 monolith，後拆分**：orchestra backend 目前是 one Docker image 跑 FastAPI + orchestra-core，這是你最省事的組合，直到流量或團隊規模逼你拆。
